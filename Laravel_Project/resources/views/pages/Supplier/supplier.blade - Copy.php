...
<div class="row">
    ...
    <div class="col-md-6" id="form_container">
        {!! Form::open(['action'=>'Supplier@store','class'=>'form-horizontal','id'=>'form_supplier']) !!}
        <div class="panel panel-default">
            <div class="panel-heading">
            	<h4 class="panel-title">{!! $page_title !!}</h4>
            </div><!-- panel-heading -->
            <div class="panel-body">
                <div class="form-group">
                    <div class="col-sm-6">
                        ...
                        {!! Form::text('txt_supplier_name','',array('class' => 'form-control','style'=>'width:100%;')) !!}
                    </div>
                    <div class="col-sm-6">
    ...
    <script>
		function fnc_set_data(id)
		{
			...
			$.ajax({
				type		: 'GET',
				url			: "{{ url('/supplier_edit/{id}') }}",
				data		: { id: id },
				success: function(data){
					//alert(data);
					//$('#form_supplier').removeAttr('action').attr('action',"{!! action('Supplier@update', ['id'=>1]) !!}");
					$('#form_supplier').removeAttr('action');
					$('#saveButton').removeAttr('type').attr('type','button').attr('onclick','fnc_update('+id+')');
					eval(data);
				},
				error: function(data){
					alert("Error Occured");
		...
		function fnc_update(id)
		{
			//alert("su..re"+id);
			var formData    = $("#form_supplier").serialize();
			$.ajax({
				type		: 'POST',
